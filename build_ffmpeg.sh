...
if [ "$(uname)" == "Darwin" ]; then
    # brew install yasm nasm automake gettext # uncomment this line afterwards: Vikas
...
export CFLAGS="-I$PREFIX/include --sysroot=$SYSROOT $OPTIMIZE_CFLAGS"
...
if [ $TARGET == 'arm-v7n' ]; then
    ...
    CPU=armv7-a
    ARCH=arm
    OPTIMIZE_CFLAGS="-mfpu=neon -mtune=cortex-a8 -march=$CPU"
